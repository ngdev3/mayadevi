...
function publisher_mapping_deatils($id){
  ...
  /**
    ...
    function checkRandomMappingEntery(){
        $this->db->select_max('billing_id');
        $query = $this->db->get('kisanvahidata')->row_array();
       // pr($query); die;
        if($query['Kisan_ID '] == ''){
            return '1';
        }else{
            return $query['Kisan_ID '];
    ...
    function get_billofsupplyData(){
        $this->db->select('*');
        $this->db->where('FY', fy()->FY);
        $this->db->where('product_type', fy()->product_type);
        $this->db->where('invoice_id',$_POST['bos_number']);
        $query = $this->db->get('invoice_system')->row();
        return $query;
    }

    function account_names(){
      
        $this->db->select('is.*, acn.*');
        $this->db->join('aa_account_name as acn',"acn.account_id =is.account_id","left");
        $query = $this->db->get('invoice_system as is')->result();
        return $query;
    }


    function account_mapping_name(){
        $this->db->select('*');
        $this->db->where('CenterName',$_POST['center_type']);
        $query = $this->db->get('kisanvahidata')->result();
      //  pr($query); die;
        return $query;
    }

    function account_kishanName(){
        $this->db->select('*');
        $this->db->where('status_rec','pending');
        $this->db->where('Farmer_ID',$_POST['farmer_id']);
        $this->db->where('CenterName',$_POST['center_type']);
        $query = $this->db->get('kisanvahidata')->row();
      //  pr($query); die;
        return $query;
    }
