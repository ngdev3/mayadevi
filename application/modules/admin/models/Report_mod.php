...
class Report_mod extends CI_Model
{
    ...
    function kishanVahi_Data_details()
    {
       ...
       // $querys =  $this->db->query("SELECT aa_account_name.name, kisanvahidata.* FROM kisanvahidata LEFT JOIN aa_account_name on aa_account_name.account_id = kisanvahidata.account_no where kisanvahidata.Purchase_Date = '" . $defaultDate . "'");
        ...
        $this->db->select('acn.name, kvd.*, regkvd.aadhar_card, centers.name as centern ');
        $this->db->join('aa_account_name as acn', 'acn.account_id = kvd.account_no', 'left');
        $this->db->join('reg_kisanvahidata as regkvd', 'regkvd.Farmer_ID = kvd.Farmer_ID', 'left');
        $this->db->join('aa_center_name as centers', 'centers.center_id = kvd.CenterName', 'left');
        $this->db->where('kvd.Purchase_Date', $defaultDate);
        $this->db->group_by('kvd.Farmer_ID'); 
        $this->db->from('kisanvahidata as kvd');
        $querys = $this->db->get();
    ...
    {
        $this->db->select('SUM(Ammount) as Amount, SUM(Quantity) as Quantity, acn.is_Kisan');
        $this->db->where('FY', fy()->FY);
        $this->db->where('product_type', fy()->product_type);
        $this->db->join("aa_account_name as acn", 'acn.account_id = kisanvahidata.account_no', 'left');
        $this->db->where('account_no', $id);
        $query = $this->db->get('kisanvahidata');
        return $query->row();
    }
    ...
    function convertUserIsKisan($data, $flag)
    {
        $up['is_Kisan'] = $flag;
        $this->db->where('account_id', $data);
        $update =  $this->db->update('aa_account_name', $up);
        $rs = $this->db->affected_rows();
        if ($rs) {
            return true;
        } else {
