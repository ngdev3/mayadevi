 ...
 * @since		Version 1.0
 */
class Taxinvoice_mod extends CI_Model {
    ...
    public function view($id){
       // pr($id); die;
        ...
        $this->db->from('tax_invoice_system is');
        $query = $this->db->get();
        if($query->num_rows() > 0){
            return $query->row();
        } else{
    ...
    function add($data){
        $this->db->insert('tax_invoice_system', $data);
    ...
    function edit($id = null, $userdata){  
        ...
        $this->db->update('tax_invoice_system',$userdata);
	...
	function count_Billing_data() {
		...
		$this->db->select('*');
        $this->db->order_by('tax_invoice_id','desc');
		...
		return $query = $this->db->get('tax_invoice_system');
    }
    ...
    function account_name(){
        ...
        $query = $this->db->get('tax_invoice_system as ab')->result();
      //  pr($query); die;
        return $query;
    }
    ...
    function get_Billing_data($parent_id = "") {  
        ...
        $this->db->select("ab.*,acn.*, ab.cgst_amount as tax_cgst_amount, ab.sgst_amount as tax_sgst_amount, ab.tax_gst_amount as tax_gst_amount, acn.name as account_name, ab.total_invoice as tax_total_invoice, ab.type_of_invoice as type_of_invoice, isp.*, ab.billing_date, isp.updated_date as isupdated_date");
        $this->db->join('invoice_system as isp', 'isp.bos_id  = ab.bos_number ','left');
        $this->db->join('aa_account_name as acn', 'acn.account_id  = isp.account_id ','left');
        $this->db->where('ab.FY', fy()->FY);
        $this->db->where('ab.product_type', fy()->product_type);
        $this->db->from("tax_invoice_system as ab");
  ...
  /**
    ...
    function get_invoice_details($id){
        ...
        $this->db->join('invoice_system as isp', 'isp.bos_id  = ab.bos_number ','left');
        $this->db->join('aa_account_name as acn', 'acn.account_id  = isp.account_id ','left');
        $this->db->where('ab.tax_invoice_id',$id);
        $this->db->where('ab.FY', fy()->FY);
        $this->db->where('ab.product_type', fy()->product_type);
        return $this->db->get("tax_invoice_system as ab")->row_array();
    }
    ...
    function find_rokadh_details($id){
        ...
        $this->db->join('aa_account_name as acn','acn.account_id = is.account_id','left');
        return $this->db->get('tax_invoice_system as ar')->row_array();
    }

    function getTaxId(){
        $this->db->select('tax_invoice_fy_id');
        $this->db->order_by('tax_invoice_fy_id','desc');
        $this->db->where('FY', fy()->FY);
        $this->db->where('product_type', fy()->product_type);
        return $this->db->get('tax_invoice_system')->row();
    }
    ...
    function delete($id){
        ...
        $this->db->where('tax_invoice_id', $id['tax_invoice_id']);
        $this->db->delete('tax_invoice_system'); 
